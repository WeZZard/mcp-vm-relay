...
class AuditVm extends VmService {
 ...
 override async release(n:string,b:{reason?:string}={},o:RequestOptions={}){
  ...
  if(lease&&!retained){
   try{if(recorder){await stopRecorder();await retainFinalizedVideo();}if(app)await stopFixture();await transfer().pullVerified(lease.workspace+'/trajectory',join(root,'application-original'));retained=true;await json('application-retention.json',{retained:true});
...
async function observe(name:string){await call('get_desktop_state',{screenshot_out_file:lease.workspace+`/trajectory/${name}.png`});await transfer().pullVerified(lease.workspace+`/trajectory/${name}.png`,join(root,`${name}.png`));const windows=await call('list_windows',{on_screen_only:true});await json(`${name}-windows.json`,windows);return windows;}
...
try{
 ...
 lease=await manager.acquire({task:process.env.MACOS_SMOKE_TASK??'relay-smoke-macos',image:'macos26',env:'none',ttlHours:1,extractions:[{path:'trajectory',name:'trajectory'},{path:'result.json',name:'result.json'}]});await json('acquire.json',lease);
 ...
 await command(['/bin/mkdir','-p',lease.workspace+'/trajectory',lease.guestRoot+'/support'],'Prepare application evidence and support directories',120,{kind:'mkdir-owned',root:lease.guestRoot});
 ...
 await call('get_desktop_state',{screenshot_out_file:lease.workspace+'/trajectory/preflight.png'});
 ...
 await command(['/usr/bin/open','/Applications/RelayClock.app','--args',lease.workspace+'/trajectory'],'Launch tested app and timing beacon once; never replay this operation');
 await command([node,'-e',`const fs=require('fs');let n=0;const t=setInterval(()=>{if(fs.existsSync(${JSON.stringify(lease.workspace+'/trajectory/clock-geometry.json')})){clearInterval(t);console.log('clock-ready')}else if(++n>100){clearInterval(t);process.exitCode=1}},100);`],'Read actual app timing beacon readiness before starting recording');
 await json('recorder-start.json',await call('start_recording',{output_dir:lease.workspace+'/trajectory',record_video:true}));recorder=true;await guard();
 const geometry=JSON.parse(await command([node,'-e',`console.log(require('fs').readFileSync(${JSON.stringify(lease.workspace+'/trajectory/clock-geometry.json')},'utf8'))`],'Read native test field pixel coordinates from harness, not accessibility activation'));
 await writeFile(join(root,'support/harness-config.json'),JSON.stringify({node,walk:lease.workspace+'/trajectory'}));
 ...
 await call('get_desktop_state',{screenshot_out_file:lease.workspace+'/trajectory/final.png'});if(reviewGates){await observe('final-visible');await waitControl('final-visible');}await guard();await stopRecorder();await retainFinalizedVideo();
 await stopFixture();
 await json('explicit-extraction.json',await manager.extract(['result.json']));
 await json('finish.json',await manager.finish());
 await json('success.json',{execution:'passed',recording:'see trajectory-review/manifest.json',humanReview:'pending'});
