...
try{
 ...
 lease=await manager.acquire({task:'linux-display-diagnostic',image:'ubuntu2404',env:'none',ttlHours:1,extractions:[{path:'trajectory',name:'trajectory'},{path:'result.json',name:'result.json'}]});await json('acquire.json',lease);await json('registry.json',{present:(await readFile(`${process.env.HOME}/AGENTS.md`,'utf8')).includes(lease.purpose)});
 ...
 await command(['node','-e',`const fs=require('fs'),cp=require('child_process'),r=${JSON.stringify(lease.guestRoot)};const fd=fs.openSync(r+'/workspace/trajectory/cua.log','a');const c=cp.spawn(r+'/support/cua-driver',['serve','--dangerously-bypass-approvals'],{env:{...process.env,...JSON.parse(fs.readFileSync(r+'/support/env.json'))},detached:true,stdio:['ignore',fd,fd]});c.unref();console.log(c.pid)`],'Start isolated driver daemon with same DISPLAY/Xauthority as persistent browser');
