...
const mediaManifest=JSON.parse(await readFile(join(root,'trajectory-review/manifest.json'),'utf8'));
for(const f of mediaManifest.files){const b=await readFile(join(root,'trajectory-review',f.path));if(b.length!==f.bytes||createHash('sha256').update(b).digest('hex')!==f.sha256)throw Error('Application package checksum mismatch');}
...
await save('finish-recovered.json',{...(old.delivered??{}),applicationPackage:join(root,'trajectory-review'),hostHarness:'existing retained enclosure recovered and destroyed after verified evidence review, without input or recorder-stop replay',humanReview:'pending'});
console.log(JSON.stringify({relay:verified,application:join(root,'trajectory-review'),cleanup:'verified',humanReview:'pending'}));
