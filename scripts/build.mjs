...
const browser = await build({ ...common, entryPoints: ['src/guest/browser.ts'], outfile: 'dist/browser.mjs' });
...
const sources = {};
for (const path of [...new Set([receiver, browser, server, doctor].flatMap(r => Object.keys(r.metafile.inputs)))].sort()) {
...
await writeFile('dist/NOTICE.txt', 'This distribution includes compiled @wezzard/relay-driver-core, @wezzard/relay-driver-host-sdk and @wezzard/relay-driver-remote-runtime code (https://github.com/WeZZard/relay-driver), and the Model Context Protocol TypeScript SDK. See build-info.json for source hashes and versions. The relay implementation was inherited from pi-vm-relay (https://github.com/WeZZard/pi-vm-relay) at commit 8990123, synchronized with its implementation through commit 0d69fc7 before that project\'s retirement, and has been maintained here as mcp-vm-relay\'s own implementation from then on.\n');
const integrity = {};
for (const name of ['server.mjs', 'receiver.mjs', 'browser.mjs', 'doctor.mjs']) integrity[name] = createHash('sha256').update(await readFile(`dist/${name}`)).digest('hex');
